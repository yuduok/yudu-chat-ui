          ...
          <TabsContent value="images" className="space-y-3">
            ...
            {Object.keys(imageDraft).map((id) => {
              ...
              return <div key={id} className="space-y-3 rounded-md border p-4">
                ...
                <div className="space-y-1.5"><Label>{t("settings.apiKey")}</Label><div className="relative"><Input aria-label={t("settings.apiKey")} type={value.show ? "text" : "password"} value={value.apiKey} onChange={(event) => setImageDraft((draft) => ({ ...draft, [id]: { ...draft[id], apiKey: event.target.value } }))} /><button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground" title={value.show ? "Hide" : "Show"} aria-label={value.show ? "Hide" : "Show"} onClick={() => setImageDraft((draft) => ({ ...draft, [id]: { ...draft[id], show: !draft[id].show } }))}>{value.show ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></div>
